...
namespace Ragnarok //this creates a namespace for all of the IvyMoon scripts so they dont interfere with yours
{
    public class InventoryBuddy : EditorWindow
    {
        ...
        [MenuItem("Window/InventoryBuddy %#e")]
        static void Init()
        {
            EditorWindow.GetWindow(typeof(InventoryBuddy));
        }
        ...
        void OnGUI()
        {
            ...
            if (inventoryItemList != null)
            {
                ...
                if (inventoryItemList.itemList.Count > 0)
                {
                    ...
                    //  GUILayout.Space(10);
                    inventoryItemList.itemList[viewIndex - 1].itemDescription = EditorGUILayout.TextField("Item Description", inventoryItemList.itemList[viewIndex - 1].itemDescription as string);

                    //inventoryItemList.itemList[viewIndex - 1].MaxStackQuantity = EditorGUILayout.IntField("Max Stack Size", inventoryItemList.itemList[viewIndex - 1].MaxStackQuantity);
        ...
        void AddItem()
        {
            InventoryItem newItem = new InventoryItem(IItem);
